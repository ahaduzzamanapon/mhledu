...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('mhledu__pages', function (Blueprint $table) {
            if (!Schema::hasColumn('mhledu__pages', 'is_default')) {
       ...
       // $in_app_live = Permission::where('module','InAppLiveClass')->delete();
        ...
        foreach($themes as $theme){
            if($theme->title == 'Default') { 
                ...
                $sql[] = ['theme_id'  => $theme->id, 'color_id' => 3, 'value'  => "#3091ff"];
        ...
        foreach ($filesInFolder as $path) {
            ...
            if (file_exists($file['dirname'] . '/' . $file['basename'])) {
                ...
                if ($file_data) {
                    $check_exist  = DB::table(config('pagebuilder.db_prefix', 'mhledu__') . 'pages')->where('school_id', 1)->where('slug', $file_data['slug'])->first();
                    if (!$check_exist) {
                        DB::table(config('pagebuilder.db_prefix', 'mhledu__') . 'pages')->insert(
                            [
                                'name' => $file_data['name'],
